...
$(document).on('click', '.reviewTradeData', function() {
    ...
    request("/economy/trades/"+id+"/items", "GET")
        .then(function(d) {
            ...
            d["requestee"].forEach(function(v) {
                itemsToGiveHtml += '<div class="col-6 col-sm-4"><a href="/catalog/'+v["catalogId"]+'" target="_blank"><img data-catalogid="'+v["catalogId"]+'" style="width: 100%;" /></a><p class="text-center text-truncate" style="font-size: 0.85rem;" data-catalogid="'+v.catalogId+'">Loading</p></div>';
                catalogThumbsArr.push(v["catalogId"]);
            });
            d["requested"].forEach(function(v) {
                itemsToRecieveHtml += '<div class="col-6 col-sm-4"><a href="/catalog/'+v["catalogId"]+'" target="_blank"><img data-catalogid="'+v["catalogId"]+'" style="width: 100%;" /></a><p class="text-center text-truncate" style="font-size: 0.85rem;" data-catalogid="'+v.catalogId+'">Loading</p></div>';
                catalogThumbsArr.push(v["catalogId"]);
            });
            if (type === "outbound") {
            ...
            Swal.fire({
                ...
                html:
                  `<div class="row">
                  <div class="col-sm-6"><img data-userid="`+partnerUserId+`" style="width:100%;" /><h5><a href="/users/`+partnerUserId+`/profile" target="_blank">`+userIdToName(partnerUserId)+`</a></h5><p style="font-size: 0.75em;">`+date+`</p></div>
                  <div class="col-sm-6">
                  <h5>`+itemsGiveMessage+`</h5>
                  <div class="row">
                    `+itemsToGiveHtml+`
                  </div>
                  <h5>`+itemsRecevMessage+`</h5>
                  <div class="row">
                    `+itemsToRecieveHtml+`
                  </div>
                  </div>
